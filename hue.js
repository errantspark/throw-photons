...
var basestation = "http://192.168.1.16/";
...
var toCart = function(r, g, b, d, e, f) {
    var sum = [r, g, b].reduce(function(a, b) {
        return a + b
    })
    r = (r / sum);
    g = (g / sum);
...
var hexToCIE = function(color, gamut) {
    ...
    //compatibilitycurry
var jankyColor = function(a) {
    return hexToCIE(a, hueGamut)
};
        ...
        if (i === 0) {
            ...
            setTimeout(
                function(n, tt) {
                    put({
                        xy: t[n],
                        transitiontime: Math.round(tt / 100)
                    });
...
var initiate = function(n) {
    ...
    var lights = [];
    for (var attrname in lindex) {
        lights.push(attrname)
    };
    lights = lights.map(function(n) {
        return new Lumi(basestation + "api/" + developerid + "/lights/" + n)
    });
    n.lights = lights;
